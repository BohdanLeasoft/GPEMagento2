...
class Klarna extends Ems
{
    ...
    public function isAvailable(CartInterface $quote = null): bool
    {
        ...
        if (!$this->configRepository->isKlarnaAllowed((int)$quote->getStoreId())) {
            return false;
        }
    ...
    public function captureOrder($order)
    {
        ...
        $testApiKey = $testModus ? $this->configRepository->getKlarnaTestApiKey($storeId, true) : null;
    ...
    public function refund(InfoInterface $payment, $amount)
    {
        ...
        if ($creditmemo->getAdjustmentPositive() != 0 || $creditmemo->getAdjustmentNegative() != 0) {
            throw new LocalizedException(
                __('Api does not accept adjustment fees for refunds using order lines')
            );
        }
        ...
        if ($creditmemo->getShippingAmount() > 0
            ...
            throw new LocalizedException(
                __('Api does not accept adjustment fees for shipments using order lines')
            );
        }
